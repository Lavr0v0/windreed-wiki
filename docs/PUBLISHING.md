...
判断故事组时使用这条顺序：长路记小队主线，际遇记个人线与支线，逸闻收纳不推进主线或个人线的独立故事。
...
- 修史室采用账号级权限：启用的编辑者可以编辑并直接发布全部现有词条与以后新增的词条，不再逐篇分配。
- 停用编辑者账号会收回修史室内部权限。
- Cloudflare Access 登录白名单与修史室内部账号是两层独立设置；新增或更正邮箱时必须同时更新两处。
...
- `本地有修改`：可以继续推送。
- `线上有修改`：按九卷目录保留到 `content/incoming/{档案组或故事组}/{卷册}/`，人工合并到本地源文件。
