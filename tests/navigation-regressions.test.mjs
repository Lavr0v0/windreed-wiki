...
test("uses the nine archive sections in canonical URLs while keeping slugs stable", async () => {
  const [content, editor, repository, manifest, richEditor] = await Promise.all([
    read("app/editor/lib/content.ts"),
    read("app/editor/components/EditorApp.tsx"),
    read("app/editor/lib/repository.server.ts"),
    read("app/archive-manifest.ts"),
    read("app/editor/components/RichEditor.tsx"),
  ]);
  ...
  assert.match(editor, /windreed\.wiki\/archive\/\{payload\.section\}\//);
  assert.match(manifest, /`\/archive\/\$\{entry\.section\}\/\$\{entry\.slug\}`/);
  assert.match(richEditor, /`\/archive\/\$\{target\.section\}\/\$\{target\.slug\}`/);
  assert.match(repository, /payload\.slug !== current\.slug/);
